...
//    [_baseView setBackgroundColor:[UIColor colorWithRed:0.5 green:0.6 blue:0.8 alpha:1]];
    [_baseView setBackgroundColor:[UIColor blackColor]];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if(!view_mask)
    {
        view_mask = [[UIView alloc] initWithFrame:_currentView.frame];
        view_mask.backgroundColor = [UIColor blackColor];
        view_mask.alpha = .5f;
        [_currentView addSubview:view_mask];
        [_currentView bringSubviewToFront:view_mask];
    }
    view_mask.alpha = 0;
}
...
- (void)pan:(UIPanGestureRecognizer*)pan{
    ...
    if (xoffset>0) {//向右滑
        ...
        if (_rightViewController && _rightViewController.view.superview) {
            ...
            CGFloat offset = fabs(xoffset);
            CGFloat percent = offset / _currentView.frame.size.width;
            percent = percent > 0.6f?0.6f:percent;
            view_mask.alpha = percent;
        }else{
            xoffset = 0;
            view_mask.alpha = 0;
        }
