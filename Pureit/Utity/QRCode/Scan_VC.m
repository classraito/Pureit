...
- (void)setupMaskView
{
    ...
    mask.bounds = CGRectMake(0, 0, self.view.sd_width + kBorderW + kMargin , self.view.sd_height);
...
{
    CGFloat scanWindowH = self.view.sd_height - kBorderW * 2;
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        if(delegate && [delegate respondsToSelector:@selector(didScanQRCode:)])
        {
            
            [delegate didScanQRCode:metadataObject.stringValue];
            [self disMiss];
        }
