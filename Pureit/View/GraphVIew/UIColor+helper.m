...
+(colorModel *)RGBWithUIColor:(UIColor *)_color
{
    CGColorRef _cgColor = _color.CGColor;
    NSInteger numComponents = CGColorGetNumberOfComponents(_cgColor);
    if(numComponents == 4)
    {
        const CGFloat *components = CGColorGetComponents(_cgColor);
        colorModel *rgb = [colorModel sharedColorModel] ;
        rgb.R = components[0];
        rgb.G = components[1];
        rgb.B = components[2];
        rgb.alpha = components[3];
        return  rgb;
    }
    return nil;
}
