...
-(void)awakeFromNib
{
    ...
    btn.titleLabel.textColor = orangeColor;
}
...
-(IBAction)action_dateChanged:(id)_sender;
{
...
{
    if(delegate && [delegate respondsToSelector:@selector(didCloseDatePicker:)])
    {
        NSDateFormatter* dateFormatter=[[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy 年 MM 月 dd 日"];
        str_year = [dateFormatter stringFromDate:dp_birthday.date];
        [delegate didCloseDatePicker:[str_year copy]];
    }
}
@end
